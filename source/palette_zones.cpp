	...
	map(nullptr)
{
	...
	zone_list = newd wxListCtrl(this, PALETTE_ZONES_LISTBOX,
...
Brush* ZonesPalettePanel::GetSelectedBrush() const
{
	...
	g_gui.zone_brush->setZone(
		item == -1 || !map->zones.hasZone(nstr(zone_list->GetItemText(item)))?
			0 :
			map->zones.getZoneID(nstr(zone_list->GetItemText(item)))
	);
